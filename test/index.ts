...
import {anchorHast} from '../src/lib/anchor/hast.js'
...
import {
  ...
  buildOfmSlugPath,
  buildOfmTargetHref
} from '../src/lib/shared/ofm-url.js'
...
test('root public API exposes OFM URL helpers', () => {
  assert.equal(publicApi.buildOfmTargetHref, buildOfmTargetHref)
  assert.equal(publicApi.buildOfmSlugPath, buildOfmSlugPath)
})
...
test('wikiLinkHast uses root-path default', () => {
...
test('wikiLinkHast preserves fragments in root-relative hrefs', () => {
  const node = createWikiLinkElement({
    value: 'Page#Heading',
    path: 'Page',
    fragment: 'Heading'
  ...
  assert.equal(node.properties.href, '/page#heading')
...
test('wikiLinkHast resolves current-page heading fragments through path candidates', () => {
  ...
  assert.equal(node.properties.href, '/current-page#heading')
...
test('wikiLinkHast uses the core target href when no current-page candidates are returned', () => {
  ...
  assert.equal(node.properties.href, '#heading')
...
test('wikiLinkHast resolves aliases using the target path rather than alias text', () => {
  ...
  assert.equal(node.properties.href, '/page')
...
test('wikiLinkHast preserves block fragments', () => {
  const node = createWikiLinkElement({
    value: 'Page#^block-id',
    path: 'Page',
    fragment: '^block-id'
  ...
  assert.equal(node.properties.href, '/page#^block-id')
...
test('wikiLinkHast slugifies nested paths and preserves fragments', () => {
  const node = createWikiLinkElement({
  ...
  assert.equal(node.properties.href, '/folder-name/page-name#heading-here')
...
test('wikiLinkHast falls back to core slugging', () => {
  const node = createWikiLinkElement({
    value: 'R&D?Notes',
    path: 'R&D?Notes'
  })
  ...
  assert.equal(node.properties.href, '/r-d-notes')
})
...
test('wikiLinkHast preserves # inside a heading fragment', () => {
  ...
  assert.equal(node.properties.href, '/page#a#b')
...
test('wikiLinkHast can skip title assignment', () => {
  ...
  wikiLinkHast({ setTitle: false })(node)

  assert.equal(node.properties.href, '/page')
  assert.equal(node.properties.title, undefined)
})

test('wikiLinkHast keeps the original path when no path candidates are returned', () => {
  const node = createWikiLinkElement({
    value: 'note',
    path: 'note'
  })
  ...
  assert.equal(node.properties.href, '/note')
  assert.equal(node.properties.title, 'note')
  assertOfmPublicProps(node, {
    kind: 'wikilink',
    path: 'note'
  })
...
test('wikiLinkHast resolves the path from the first returned candidate', () => {
  const node = createWikiLinkElement({
    value: 'note',
    path: 'note'
  })
  ...
  assert.equal(node.properties.href, '/learning/note')
  assert.equal(node.properties.title, 'learning/note')
  assertOfmPublicProps(node, {
    kind: 'wikilink',
    path: 'learning/note'
  })
...
test('wikiLinkHast uses the first path candidate when multiple matches are returned', () => {
  const node = createWikiLinkElement({
    value: 'note',
    path: 'note'
  })
  ...
  wikiLinkHast({
    resolvePathCandidates(path) {
      assert.equal(path, 'note')
      return ['learning/note', 'work/note']
    }
  })(node)
...
test('wikiLinkHast resolves multi-segment paths through path candidates', () => {
  const node = createWikiLinkElement({
    value: 'learning/note',
    path: 'learning/note'
  })
  ...
  wikiLinkHast({
    resolvePathCandidates(path) {
      assert.equal(path, 'learning/note')
      return ['archive/learning/note']
    }
  })(node)

  assert.equal(node.properties.href, '/archive/learning/note')
  assert.equal(node.properties.title, 'archive/learning/note')
  assertOfmPublicProps(node, {
    kind: 'wikilink',
    path: 'archive/learning/note'
  })
})
...
test('embedHast renders markdown file embeds as semantic containers with fragments', () => {
  ...
  assert.deepEqual(node.children, [{
    type: 'element',
    tagName: 'a',
    properties: {href: '/page-md#heading'},
...
test('embedHast renders image embeds as images', () => {
  ...
  assert.equal(node.properties.src, '/assets/cover-png')
...
test('embedHast renders non-image files as links', () => {
  ...
  assert.equal(node.properties.href, '/manual-pdf')
...
test('embedHast uses the target path rather than alias text for note embed title', () => {
  ...
  assert.deepEqual(node.children, [{
    type: 'element',
    tagName: 'a',
    properties: {href: '/page'},
    children: [{type: 'text', value: 'Alias'}]
  }])
})

test('embedHast preserves block fragments for note embeds', () => {
  const node = createEmbedElement({
    value: 'Page#^block-id',
    path: 'Page',
    fragment: '^block-id'
  ...
  assert.deepEqual(node.children, [{
    type: 'element',
    tagName: 'a',
    properties: {href: '/page#^block-id'},
    children: [{type: 'text', value: 'Page#^block-id'}]
  }])
  assert.equal(node.properties.title, 'Page#^block-id')
})

test('embedHast slugifies nested paths and preserves fragments', () => {
  const node = createEmbedElement({
    value: 'Folder Name/Page Name#Heading Here',
    path: 'Folder Name/Page Name',
    fragment: 'Heading Here'
  ...
  assert.deepEqual(node.children, [{
    type: 'element',
    tagName: 'a',
    properties: {href: '/folder-name/page-name#heading-here'},
    children: [{type: 'text', value: 'Folder Name/Page Name#Heading Here'}]
  }])
})

test('embedHast falls back to core slugging', () => {
  const node = createEmbedElement({
    value: 'R&D?Notes',
    path: 'R&D?Notes'
  })
...
test('embedHast can skip title assignment', () => {
  ...
  embedHast({ setTitle: false })(node)

  assert.equal(node.tagName, 'div')
  assert.equal(node.properties.title, undefined)
})

test('embedHast keeps the original path when no path candidates are returned', () => {
  const node = createEmbedElement({
    value: 'note',
    path: 'note'
  })
  ...
  assert.deepEqual(node.children, [{
    type: 'element',
    tagName: 'a',
    properties: {href: '/note'},
    children: [{type: 'text', value: 'note'}]
  }])
})

test('embedHast resolves the path from the first returned candidate', () => {
  const node = createEmbedElement({
    value: 'note',
    path: 'note'
  })
  ...
  assert.deepEqual(node.children, [{
    type: 'element',
    tagName: 'a',
    properties: {href: '/learning/note'},
    children: [{type: 'text', value: 'learning/note'}]
  }])
})

test('embedHast uses the first path candidate when multiple matches are returned', () => {
  const node = createEmbedElement({
    value: 'note',
    path: 'note'
  })
...
test('embedHast resolves multi-segment paths through path candidates', () => {
  const node = createEmbedElement({
    value: 'learning/note',
    path: 'learning/note'
  })
  ...
  assert.deepEqual(node.children, [{
    type: 'element',
    tagName: 'a',
    properties: {href: '/archive/learning/note'},
    children: [{type: 'text', value: 'archive/learning/note'}]
  }])
})
...
test('rendering note embeds preserves fragment metadata and slug href output', () => {
  ...
  assert.deepEqual(node.children, [{
    type: 'element',
    tagName: 'a',
    properties: {href: '/project-notes#overview'},
...
test('rendering block embeds preserves fragment and block metadata', () => {
  ...
  assert.deepEqual(node.children, [{
    type: 'element',
    tagName: 'a',
    properties: {href: '/roadmap#^next-step'},
...
test('rehypeOfm keeps block anchor props on the last paragraph after note embed splitting', () => {
  ...
  assert.equal(trailing.properties.id, '^block-id')
...
test('anchorHast adds id to headings', () => {
  const node: Element = {
    type: 'element',
    tagName: 'h2',
    properties: {},
  ...
  assert.equal(node.properties.id, 'heading-here')
  assert.equal(node.properties['data-ofm-kind'], undefined)
  assert.equal(node.properties['data-ofm-variant'], undefined)
  assertClassNames(node, [ofmClassNames.headingTarget])
})
...
test('anchorHast extracts trailing block refs and renders labels by default', () => {
  ...
  assert.equal(node.properties.id, '^block-id')
  assert.equal(node.properties['data-ofm-kind'], undefined)
  assert.equal(node.properties['data-ofm-variant'], undefined)
  assert.equal(node.properties['data-ofm-block-id'], 'block-id')
  assertClassNames(node, [ofmClassNames.blockTarget])
  assert.deepEqual(node.children, [
...
test('anchorHast preserves existing class names on block targets', () => {
  ...
  assertClassNames(node, ['existing-item', ofmClassNames.blockTarget])
})
...
test('buildOfmSlugPath generates the canonical slug path without a prefix', () => {
  assert.equal(buildOfmSlugPath(' Folder%20Name / Page%20Name '), 'folder-20name/page-20name')
  assert.equal(buildOfmSlugPath('学习/first-map'), '学习/first-map')
  assert.equal(buildOfmSlugPath(' Folder Name / R&D?Notes '), 'folder-name/r-d-notes')
})
...
test('buildOfmTargetHref normalizes paths and slugifies heading or block fragments', () => {
  ...
  assert.equal(
    ...
    '/folder-20name/page-20name#heading-here'
  )
...
test('buildOfmTargetHref slugifies mixed case, spaces, and symbols per path segment', () => {
  assert.equal(
    buildOfmTargetHref({
        path: ' Folder Name / R&D?Notes '
      }),
    '/folder-name/r-d-notes'
  )
