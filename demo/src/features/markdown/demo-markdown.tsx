...
function createDemoMarkdownPreset(options: OfmReactPresetOptions = {}): DemoMarkdownPreset {
  const preset = createOfmReactPreset({
    ...
    wikiLink: {
      resolve(target) {
        return {
          href: buildDemoWikiHref(target.path, target.blockId ? `^${target.blockId}` : target.fragment),
...
function buildDemoTitle(target: OfmReactTarget, fallbackTitle?: string): string {
  if (fallbackTitle) {
    return fallbackTitle
  }

  if (target.blockId) {
    return `${target.path}#^${target.blockId}`
  }
